...
import com.edugroupe.introductionBoot.dao.ProduitDAO;
import com.edugroupe.introductionBoot.metier.Produit;

@Controller
public class ProduitController {

	//Autowired = injecte des données dans Produit
	@Autowired
	...
	public String bonjour(Model model) 
	{
		...
		model.addAttribute("titre", p.getNom());
		model.addAttribute("prix","Prix : " + p.getPrix()+ " euros");
		model.addAttribute("poids","Poids : " + p.getPoids());
		
		Produit p1 = produitDAO.findById(2);
		model.addAttribute("titre1",p1.getNom());
		model.addAttribute("prix1","Prix : " + p1.getPrix()+ " euros");
		model.addAttribute("poids1","Poids : " + p1.getPoids());
		
		Produit p2 = produitDAO.findById(3);
		model.addAttribute("titre2", p2.getNom());
		model.addAttribute("prix2","Prix : " + p2.getPrix() + " euros");
		model.addAttribute("poids2","Poids : " + p2.getPoids());
		//model.addAttribute("message","Bienvenue le " + LocalDateTime.now());
		return "home";
	}
	...
	public ModelAndView listeProduit() {
		//ModelAndView est une classe de spring combinant  la sélection d'une vue(jsp par exemple)
		//avec le stockage des données destinées à cette vue
	...
	//elle apparait dans l'irl en {}, les accolades ne sont pas présente dans la vrai url
